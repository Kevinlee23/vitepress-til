...
server {
    ...
    # 这两个是 ssl 的私钥和公钥文件
    ssl_certificate /etc/nginx/cert/memos.snowinlu.top.pem;
    ssl_certificate_key /etc/nginx/cert/memos.snowinlu.top.key;

    ssl_protocols TLSv1.2 TLSv1.3; # 支持的协议版本
    ssl_ciphers HIGH:!aNULL:!MD6; # 加密套件

    location / {
        root /usr/local/project/hexo-stellar-archiv;
...
server {
    listen 80;
    server_name memos.snowinlu.top;
    return 301 https://$host$request_uri; # HTTP 重定向到 HTTPS
}
...
server {
    ...
    # 这两个是 ssl 的私钥和公钥文件
    ssl_certificate /etc/nginx/cert/memos2.snowinlu.top.pem;
    ssl_certificate_key /etc/nginx/cert/memos2.snowinlu.top.key;
